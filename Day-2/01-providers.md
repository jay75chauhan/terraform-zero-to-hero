...
In this example, we are first defining the aws provider. We are specifying the region as ap-south-1. Then, we are defining the `aws_instance` resource. We are specifying the `AMI ID` and the `instance type`.
